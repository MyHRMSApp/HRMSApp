...
export class MyRequestPage {
  ...
  ionViewDidLoad() {
    this.hamburger = (this.commonString.commonStrings.MyRequestsPage.HAMBURGERICON_IMG);
    this.homeIcon = (this.commonString.commonStrings.MyRequestsPage.HOMEICON_IMG);
  }
  ...
  confirmCancelLeave(status, requestItem, leaveType) {
    ...
    if (status == 'P') {
      ...
      let deletionTextareaAlert = this.modalCtrl.create(this.commonString.commonStrings.MyRequestsPage.ALERTPAGETEXTAREA);
  ...
  applyCancelDeleteRequest(payloadData) {
    ...
    this.utilService.showLoader(this.commonString.commonStrings.MyRequestsPage.PLEASE_WAIT);
    console.log(payloadData);
    this.service.invokeAdapterCall('commonAdapterServices', 'applyCancelandDeleteRequest', 'post', {
      payload: true,
      length: 4,
      ...
      if (resultData) {
        if (resultData.status_code == 0) {
          if (resultData.data.FLAG == "E") {
            ...
            this.utilService.showCustomPopup4Error("My Request", resultData.data.REASON, this.commonString.commonStrings.ProfilePage.FAILURE_TITLE_TEXT);
          ...
          this.utilService.dismissLoader();
          this.utilService.showCustomPopup4Error("My Request", resultData.message, this.commonString.commonStrings.ProfilePage.FAILURE_TITLE_TEXT);
        }
